...
@Client.on_message(command(["vplay", "vplay@Tg_Vc_00_Bot"]))
async def play(_, message: Message):
    ...
    audio = (message.reply_to_message.video or message.reply_to_message.document) if message.reply_to_message else None
    url = get_url(message)
    ...
    if audio:
        ...
        mystic = await message.reply_text(f"**🔄 Processing Video Given By {username}**")
        ...
        title = "Selected Video from Telegram"
        ...
        audio_file = await convert(x)
        video_file = await convert(x)
    else:
        if len(message.command) < 2:
            ...
            photo=thumb, 
            caption=("**Usage:** /play [Video Name or Youtube Link or Reply to Video]\n\nIf you want to play Playlists! Select the one from Below."),    
...
@app.on_message(filters.command("vplayplaylist"))
async def play_playlist_cmd(_, message):
    ...
    await message.reply_photo(
    photo=thumb, 
    caption=("**__Music's Playlist Feature__**\n\nSelect the Video Playlist you want to play!."),    
    reply_markup=InlineKeyboardMarkup(buttons),
    )
    return
